...
// Visitor-facing version of the chatbot — hits the public, no-auth chatbot
// endpoints so people browsing the site before signing up can still get
// quick answers and find their way around.
export default function PublicChatWidget({ open: controlledOpen, onOpenChange } = {}) {
  ...
  const [messages, setMessages] = useState([
    { from: 'bot', text: "Hi, I'm Evo! Ask me anything about Evolve Gym, or tap a question below." },
  ]);
  const [input, setInput] = useState('');
  const [asking, setAsking] = useState(false);

  useEffect(() => {
    if (open && faqs.length === 0 && GYM_ID) {
      api.get(`/chatbot/public/faqs?gymId=${GYM_ID}`).then(({ data }) => setFaqs(data)).catch(() => {});
    }
  }, [open]);
  ...
  async function handleSend(e) {
    ...
    setAsking(true);
    try {
      const { data } = await api.post(`/chatbot/public/ask?gymId=${GYM_ID}`, { message: text });
